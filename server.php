...
if ($action === "run" || $action === "log" || $action === "check") {
    // Get feeder's logs
    ...
    switch ($action){
        ...
        case $action == "check":
            ...
            if (count($donates) != 0 && $log->tankLevel > 0 && $log->plateLevel <= 2) {
                ...
                $stmt = $db->prepare("UPDATE petai.feeder_data SET refillsTotal = feeder_data.refillsTotal + 1 WHERE feeder=:id");
                $stmt->bindParam(':id', $feeder->id, PDO::PARAM_INT);
                $stmt->execute();
                return print "true";
            }
